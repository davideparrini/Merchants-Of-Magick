import {
  ...
  function createGameLogic() {
    ...
    function gameInit(players: string[], config: GameInitConfig): { quests: { quest1: Quest; quest2: Quest }; dices: DiceRolls; players: PlayerStartGame[]; config: GameInitConfig } {
    ...
    function compareByGold(a: SignedFinalReport, b: SignedFinalReport): number {
      return b.report.gold - a.report.gold;
    }
  
    function calculateGold(finalReport: SignedFinalReport[]): SignedFinalReport[] {
      const newFinalReport: SignedFinalReport[] = [];
    ...
    function winnerResolution(finalReport: SignedFinalReport[]): ResolvedFinalReport[] {
  ...
  export const gameLogicService = createGameLogic();